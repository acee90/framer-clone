...
function ParallaxComponent() {
	...
	return (
		...
		<main ref={mainRef} className="relative overflow-hidden bg-black">
			{/* 2. 배경 레이어 (z-10) */}
			{/* 이 div는 이제 main 래퍼에 속하며 모든 섹션의 배경이 됩니다. */}
			<div
				ref={backgroundRef}
				className="absolute top-[-30%] left-0 z-10 h-[120%] w-full bg-[url('https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1600&q=80')] bg-center bg-cover"
			...
			<section
				...
				className="relative z-20 flex h-screen w-full items-center justify-center overflow-hidden" // ✅ z-index 추가
			>
				<h1
					ref={textRef}
					className="text-center font-bold text-[7vw] text-white drop-shadow-lg"
				>
					GSAP Parallax
				</h1>
			</section>
			...
			{/* 4. 두 번째 콘텐츠 섹션 (z-20) */}
			<section
				className="relative z-20 flex h-screen flex-col items-center justify-center p-10 text-center" // ✅ z-index 추가
				// ✅ 불투명한 bg-gray-900 제거
			>
				{/* (선택 사항) 텍스트 가독성을 위한 반투명 오버레이 */}
				<div className="absolute inset-0 z-[-1] bg-gray-900/70"></div>

				<h2 className="relative font-bold text-4xl text-white">Scroll Down</h2>
